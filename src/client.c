...
void client_configure(struct client *c) {
	XConfigureEvent ev = { .type = ConfigureNotify, .event = c->win, .window =
			c->win, .x = c->rgeo.x + c->border, .y = c->rgeo.y + c->tbarw,
			.width = c->wgeo.w, .height = c->wgeo.h, .above = None,
...
void client_swap(struct client *c, enum position p) {
	...
	XGrabKeyboard(W->dpy, W->root, True, GrabModeAsync, GrabModeAsync,
			CurrentTime);
	_REV_SBORDER()
	;
	...
	do {
		...
		if (ev.type == KeyPress) {
			...
			if (k->keysym == keysym
					&& KEYPRESS_MASK(k->mod) == KEYPRESS_MASK(ke->state)
					&& k->func) {
...
static void client_grabbuttons(struct client *c, bool focused) {
	...
	if (focused) {
		...
		while (i++ != Button5) {
			...
			ButtonMask, GrabModeAsync, GrabModeSync, None, None);
			XGrabButton(W->dpy, i, W->client_mod | W->numlockmask, c->win,
					False,
					ButtonMask, GrabModeAsync, GrabModeSync, None, None);
			XGrabButton(W->dpy, i, W->client_mod | LockMask | W->numlockmask,
					c->win, False,
					ButtonMask, GrabModeAsync, GrabModeSync, None, None);
		}
	...
	/* Tabbing case, multiple titlebar in frame */
		...
		SLIST_FOREACH(cc, &c->tag->clients, tnext)
		{
			...
			if (cc->tabmaster == c) {
				...
				barwin_draw_text(cc->titlebar, c->theme, xt, y - 1, c->ncol.fg,
						title);
...
void _client_tab(struct client *c, struct client *cm) {
	...
	if (c->flags & (CLIENT_TABBED | CLIENT_TABMASTER) || c->tag != cm->tag
	|| c == cm
	|| !COMPCLIENT(c, cm))
		return;
...
static void client_untab(struct client *c) {
...
void uicb_client_focus_click(Uicb cmd) {
	(void) cmd;
	struct client *c;

	if ((c = client_gb_titlebar(W->last_clicked_barwin->win)) || (c =
			client_gb_frame(W->last_clicked_barwin->win)))
		client_focus(c);
}
...
void client_get_name(struct client *c) {
	...
	if (XGetWindowProperty(W->dpy, c->win, W->net_atom[net_wm_name], 0, 65536,
			False, W->net_atom[utf8_string], &rt, &rf, &ir, &il,
			(unsigned char**) &c->title) != Success)
...
void client_get_sizeh(struct client *c) {
	...
	if (c->sizeh[MAXW] && c->sizeh[MINW] && c->sizeh[MAXH] && c->sizeh[MINH]
			&& c->sizeh[MAXW] == c->sizeh[MINW]
			&& c->sizeh[MAXH] == c->sizeh[MINH])
		c->flags |= CLIENT_HINT_FLAG;
}

static void client_frame_new(struct client *c) {
	struct barwin *frameb;
	struct barwin *clientb;
	XSetWindowAttributes at = { .background_pixel = c->ncol.bg,
			.override_redirect = true, .background_pixmap = ParentRelative,
	...
	frameb->win = c->frame = XCreateWindow(W->dpy, W->root, c->geo.x, c->geo.y,
			c->geo.w, c->geo.h, 0, CopyFromParent, InputOutput, CopyFromParent,
	...
	if (c->tbarw > c->border) {
		c->titlebar = barwin_new(c->frame, 0, 0, 1, c->tbarw, c->ncol.fg,
				c->ncol.bg, true);
...
static void _apply_rule(struct client *c, struct rule *r) {
	if (r->screen != -1)
...
static void client_apply_rule(struct client *c) {
	struct rule *r;
	...
	/* Get WM_WINDOW_ROLE */
	if (XGetWindowProperty(W->dpy, c->win, ATOM("WM_WINDOW_ROLE"), 0L,
			0x7FFFFFFFL, false, XA_STRING, &rf, &f, &n, &il, &data) == Success
			&& data) {
	...
	if (XGetWindowProperty(W->dpy, c->win, ATOM("WM_CLIENT_MACHINE"), 0L,
			0x7FFFFFFFL, false, XA_STRING, &rf, &f, &n, &il, &data) == Success
			&& data) {
	...
	/* Get _NET_WM_NAME */
	if (XGetWindowProperty(W->dpy, c->win, W->net_atom[net_wm_name], 0,
			0x77777777, false, W->net_atom[utf8_string], &rf, &f, &n, &il,
			&data) == Success && data) {
	...
	SLIST_FOREACH(r, &W->h.rule, next)
	{
		...
		if (s) {
			FLAGAPPLY(flags,
					(xch.res_name && r->instance
							&& !strcmp(xch.res_name, r->instance)), RINSTANCE);
			FLAGAPPLY(flags,
					(xch.res_class && r->class
							&& !strcmp(xch.res_class, r->class)), RCLASS);
		}

		FLAGAPPLY(flags, (wmname && r->name && !strcmp(wmname, r->name)),
				RNAME);
		FLAGAPPLY(flags,
				((role && r->role && !strcmp(role, r->role)) || !role
						|| !r->role), RROLE);
		FLAGAPPLY(flags,
				(wmclientmachine && r->client_machine
						&& !strcmp(wmclientmachine, r->client_machine)),
				RCMACHINE);
...
void client_update_props(struct client *c, Flags f) {
	...
	if (f & CPROP_GEO) {
		long g[4] = { (long) c->geo.x, (long) c->geo.y, (long) c->geo.w,
				(long) c->geo.h };
...
bool client_winsize(struct client *c, struct geo *g) {
...
void _client_moveresize(struct client *c, struct geo *g,
	...
	XMoveResizeWindow(W->dpy, c->frame, c->rgeo.x, c->rgeo.y, c->rgeo.w,
			c->rgeo.h);

	if (!(c->flags & CLIENT_FULLSCREEN))
		XMoveResizeWindow(W->dpy, c->win, c->wgeo.x, c->wgeo.y, c->wgeo.w,
				c->wgeo.h);
...
void client_place_at_mouse(struct client *c) {
	...
	c->geo.x = (
			(x + c->geo.w) > c->screen->ugeo.w ?
					c->screen->ugeo.w - c->geo.w : x);
	c->geo.y = (
			(y + c->geo.h) > c->screen->ugeo.h ?
					c->screen->ugeo.h - c->geo.h : y);
}
 ...
 * manual tile layout.
 */
static inline void _fac_apply(struct client *c, enum position p, int fac) {
	switch (p) {
	case Top:
		c->tgeo.y -= fac;
	case Bottom:
...
static inline void _fac_arrange_row(struct client *c, enum position p, int fac) {
	struct geo g = c->tgeo;
	struct client *cc;
...
static inline void _fac_check_to_reverse(struct client *c) {
	struct client *gc, *cc;
	...
	FOREACH_NFCLIENT(gc, &c->tag->clients, tnext)
		if (gc->flags & CLIENT_FAC_APPLIED && client_winsize(gc, &gc->tgeo)) {
			/*
