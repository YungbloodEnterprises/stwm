...
static void config_mouse_section(struct mbhead *mousebinds,
		struct conf_sec **sec) {
	struct mousebind *m;
	int i = 0;
	char *p;
...
static void config_theme(void) {
	...
	for (i = 0; i < n; ++i) {
		...
		/* bars */
		t->bars.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "bars_fg").str);
		t->bars.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#222222", "bars_bg").str);
		t->bars_width = fetch_opt_first(ks[i], "12", "bars_width").num;

		/*
		 * Elements
		 */
		t->tags_n.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "tags_normal_fg").str);
		t->tags_n.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#222222", "tags_normal_bg").str);
		t->tags_s.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#222222", "tags_sel_fg").str);
		t->tags_s.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "tags_sel_bg").str);
		t->tags_o.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "tags_occupied_fg").str);
		t->tags_o.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#444444", "tags_occupied_bg").str);
		t->tags_u.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#444444", "tags_urgent_fg").str);
		t->tags_u.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#CC4444", "tags_urgent_bg").str);
		t->tags_border_col = bgcolor_atoh(
				fetch_opt_first(ks[i], "#888888", "tags_border_color").str);
		t->tags_border_width =
				fetch_opt_first(ks[i], "0", "tags_border_width").num;
		...
		ISTRDUP(t->tags_n_sl.status,
				fetch_opt_first(ks[i], "", "tags_normal_statusline").str);
		ISTRDUP(t->tags_s_sl.status,
				fetch_opt_first(ks[i], "", "tags_sel_statusline").str);
		ISTRDUP(t->tags_o_sl.status,
				fetch_opt_first(ks[i], "", "tags_occupied_statusline").str);
		ISTRDUP(t->tags_u_sl.status,
				fetch_opt_first(ks[i], "", "tags_urgent_statusline").str);
		...
		/* Client / frame */
		t->client_n.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "client_normal_fg").str);
		t->client_n.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#222222", "client_normal_bg").str);
		t->client_s.fg = fgcolor_atoh(
				fetch_opt_first(ks[i], "#222222", "client_sel_fg").str);
		t->client_s.bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#CCCCCC", "client_sel_bg").str);
		t->frame_bg = bgcolor_atoh(
				fetch_opt_first(ks[i], "#555555", "frame_bg").str);
		t->client_titlebar_width = fetch_opt_first(ks[i], "12",
				"client_titlebar_width").num;
		t->client_border_width = fetch_opt_first(ks[i], "1",
				"client_border_width").num;
		...
		ISTRDUP(t->client_n_sl.status,
				fetch_opt_first(ks[i], "", "client_normal_statusline").str);
		ISTRDUP(t->client_s_sl.status,
				fetch_opt_first(ks[i], "", "client_sel_statusline").str);
		ISTRDUP(t->client_f_sl.status,
				fetch_opt_first(ks[i], "", "client_free_statusline").str);
...
static void config_tag(void) {
	...
	for (i = 0; i < n; ++i) {
		...
		if (screenid == s->id || screenid == -1) {
			...
			ISTRDUP(t->statusctx.status,
					fetch_opt_first(ks[i], "", "statusline").str);
...
static void config_client(void) {
	...
	W->client_mod = modkey_keysym(
			fetch_opt_first(sec, "Super", "key_modifier").str);
...
static void config_rule(void) {
	...
	for (i = 0; i < n; ++i) {
		...
		ISTRDUP(r->client_machine,
				fetch_opt_first(ks[i], "", "client_machine").str);

		r->screen = fetch_opt_first(ks[i], "-1", "screen").num;
		r->tag = fetch_opt_first(ks[i], "-1", "tag").num;

		FLAGAPPLY(r->flags, fetch_opt_first(ks[i], "false", "free").boolean,
				RULE_FREE);
		FLAGAPPLY(r->flags, fetch_opt_first(ks[i], "false", "tab").boolean,
				RULE_TAB);
		FLAGAPPLY(r->flags,
				fetch_opt_first(ks[i], "false", "ignore_tag").boolean,
				RULE_IGNORE_TAG);
...
static void config_launcher(void) {
	struct conf_sec *sec, **ks;
	struct launcher *l;
	int n, i;
...
static void config_keybind(void) {
	int i, n;
	size_t j;
	struct conf_sec *sec, **ks;
	struct opt_type *opt;
...
void config_init(void) {
	if (get_conf(W->confpath) == -1) {
		...
		if (get_conf(W->confpath) == -1) {
			warnxl("parsing default configuration file (%s) failed.",
					W->confpath);
			sprintf(W->confpath, "%s/stwm/stwmrc", XDG_CONFIG_DIR);

			if (get_conf(W->confpath) == -1)
				errxl(1, "parsing system configuration file (%s) failed.",
						W->confpath);
		}
	}
