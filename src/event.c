...
static void event_buttonpress(XEvent *e) {
	...
	if ((c = client_gb_win(ev->window))
			&& c != W->client && ev->button == 1 && W->cfocus & CFOCUS_CLICK)
		client_focus(c);
}
...
static void event_enternotify(XEvent *e) {
	...
	if ((c = client_gb_win(ev->window)) || (c = client_gb_frame(ev->window))) {
...
static void event_clientmessageevent(XEvent *e) {
	...
	if (ev->window == W->systray.win && type == net_system_tray_opcode) {
		...
		if (ev->data.l[4]) {
			...
			if (type == stwm_function || type == stwm_cmd) {
				...
				if (XGetWindowProperty(EVDPY(e), W->root,
						W->net_atom[stwm_function], 0, 65536, False,
						W->net_atom[utf8_string], &rt, &d, &len, &il, &ret)
						== Success && ret
						&& ((func = uicb_name_func((char*) ret)))) {
					if (XGetWindowProperty(EVDPY(e), W->root,
							W->net_atom[stwm_cmd], 0, 65536, False,
							W->net_atom[utf8_string], &rt, &d, &len, &il,
							&ret_cmd) == Success && len && ret_cmd) {
						func((Uicb) ret_cmd);
						XFree(ret_cmd);
					} else
						func(NULL);
...
static void event_configureevent(XEvent *e) {
...
static void event_maprequest(XEvent *e) {
	...
	if (!XGetWindowAttributes(EVDPY(e), ev->window, &at) || at.override_redirect
...
static void event_mappingnotify(XEvent *e) {
...
static void event_propertynotify(XEvent *e) {
	...
	if ((c = client_gb_win(ev->window))) {
		...
		case XA_WM_HINTS:
			if ((h = XGetWMHints(EVDPY(e), c->win)) && (h->flags & XUrgencyHint)
...
static void event_unmapnotify(XEvent *e) {
	...
	if ((c = client_gb_win(ev->window)) && ev->send_event
		...
		if (XGetWindowProperty(EVDPY(e), c->win, W->net_atom[wm_state], 0, 2,
				False, W->net_atom[wm_state], &rt, &d, &n, &il, &ret)
				== Success)
			if (*ret == NormalState)
				client_remove(c);
	} else if ((s = systray_find(ev->window))) {
...
static void event_expose(XEvent *e) {
...
static void event_mapnotify(XEvent *e) {
	XMapEvent *ev = &e->xmap;
	struct client *c;
	struct _systray *s;
...
static void event_selectionclearevent(XEvent *ev) {
...
static void event_dummy(XEvent *e) {
	/*  printf("%d\n", e->type);*/
	(void) e;
}
