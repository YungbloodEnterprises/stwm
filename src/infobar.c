...
} elem_funcs[] = { { 't', infobar_elem_tag_init, infobar_elem_tag_update }, {
		's', infobar_elem_status_init, infobar_elem_status_update }, { 'y',
		infobar_elem_systray_init, infobar_elem_systray_update }, { 'l',
		infobar_elem_launcher_init, infobar_elem_launcher_update }, { '\0',
		NULL, NULL } };
...
static void infobar_elem_tag_init(struct element *e) {
	...
	if (SLIST_EMPTY(&e->bars)
		...
		TAILQ_FOREACH(t, &e->infobar->screen->tags, next)
		{
			...
			/* Init barwin */
			b = barwin_new(e->infobar->bar->win, j, 0, s, e->geo.h,
					e->infobar->theme->bars.fg, 0, false);
			...
			if (e->infobar->theme->tags_border_width) {
				XSetWindowBorder(W->dpy, b->win,
						e->infobar->theme->tags_border_col);
				XSetWindowBorderWidth(W->dpy, b->win,
						e->infobar->theme->tags_border_width);
			}
...
static void infobar_elem_status_init(struct element *e) {
	...
	e->geo.w = e->infobar->geo.w - e->geo.x
			- (en ? e->infobar->geo.w - en->geo.x : 0);

	if (!(b = SLIST_FIRST(&e->bars))) {
		b = barwin_new(e->infobar->bar->win, e->geo.x, 0, e->geo.w, e->geo.h,
				e->infobar->theme->bars.fg, 0, false);
		barwin_refresh_color(b);
...
static void infobar_elem_launcher_update(struct element *e) {
	...
	barwin_draw_text(b, e->infobar->theme, 1,
			TEXTY(e->infobar->theme, e->geo.h), b->fg, e->data);
...
static void infobar_elem_init(struct infobar *i) {
	...
	if (s) {
		...
		for (k = l - 1; k >= n; --k) {
			/* Only one status */
			if (i->elemorder[k] == 's'
					|| (i->elemorder[n] == 'y' && W->flags & stwm_SYSTRAY))
				continue;
...
struct infobar*
infobar_new(struct screen *s, char *name, struct theme *theme, enum barpos pos,
		const char *elem) {
	bool map;
