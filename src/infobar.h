...
enum {
	ElemTag = 0, ElemStatus, ElemSystray, ElemLauncher, ElemCustom, ElemLast
};

struct infobar *infobar_new(struct screen *s, char *name, struct theme *theme,
		enum barpos pos, const char *elem);
...
static inline void infobar_elem_placement(struct element *e) {
	...
	if (e->align == Left)
		...
		e->geo.x = (
				(p = TAILQ_NEXT(e, next)) ?
						p->geo.x - e->geo.w : e->infobar->geo.w - e->geo.w);
}

/* Bars placement management and usable space management */
static inline bool infobar_placement(struct infobar *i, enum barpos p) {
	i->pos = p;
	i->geo = i->screen->ugeo;
...
static inline void infobar_elem_screen_update(struct screen *s, int type) {
	struct infobar *i;
