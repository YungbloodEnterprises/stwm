...
static int qsort_string_compare(const void * a, const void * b) {
		...
		while ((content = readdir(dir))) {
			...
			if (filepath && stat(filepath, &st) != -1) {
				...
				if (S_ISDIR(st.st_mode))
					xasprintf(&namelist[count - 1], "%s/",
							content->d_name + strlen(p));
				else
...
static void complete_cache_free(struct launcher_ccache *cache) {
	int i;
...
static void launcher_process(struct launcher *l) {
	struct infobar *ib;
	struct element *e;
