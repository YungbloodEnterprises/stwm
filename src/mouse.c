...
static void mouse_resize(struct client *c) {
	...
	XQueryPointer(W->dpy, W->root, &w, &w, &ox, &oy, &d, &d,
			(unsigned int *) &u);
	XGrabServer(W->dpy);
	...
	do {
		...
		if (c->flags & CLIENT_FREE) {
			...
			c->geo.w = (
					(mx - c->geo.x <= c->sizeh[MINW] + c->border + c->border) ?
							c->sizeh[MINW] + c->border + c->border :
							mx - c->geo.x);
			c->geo.h = (
					(my - c->geo.y <= (c->sizeh[MINH] + c->tbarw + c->border)) ?
...
mouse_drag_tag(struct client *c, Window w) {
	...
	if (b->win == rw && (t = (struct tag*) b->ptr) && t != c->tag)
		return t;

	return NULL;
}

void mouse_move(struct client *c, void (*func)(struct client*, struct client*)) {
	...
	do {
		...
		if (!func && c->flags & CLIENT_FREE) {
			...
			XQueryPointer(W->dpy, W->root, &w, &cw, &x, &y, &d, &d,
					(uint *) &u);

			if ((c2 = client_gb_win(cw)) || (c2 = client_gb_frame(cw)) || (c2 =
					client_gb_titlebar(cw))) {
				if (c2 != last) {
					_REV_SBORDER(last);
					_REV_SBORDER(c2);
					last = c2;
...
void uicb_mouse_resize(Uicb cmd) {
	(void) cmd;
...
void uicb_mouse_move(Uicb cmd) {
	(void) cmd;

	if (W->client && mouse_check_client(W->client))
		mouse_move(W->client,
				(W->client->flags & CLIENT_FREE ? NULL : client_swap2));
}

void uicb_mouse_tab(Uicb cmd) {
	(void) cmd;
...
void uicb_mouse_integrate(Uicb cmd) {
	(void) cmd;
