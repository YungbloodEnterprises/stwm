 ...
 * Copyright (c) 2018 Kevin Hoos <kevin@yungblood.com>
 *
...
enum keyword_t {
	SEC_START, SEC_END, INCLUDE, WORD, EQUAL, LIST_START, LIST_END, NONE
};
...
push_keyword(struct keyword *tail, enum keyword_t type, char *buf,
		size_t *offset, struct files *file, int line) {
	struct keyword *kw;
#ifdef DEBUG
	int i = 0;
#endif
...
static void syntax(struct keyword *kw, const char *fmt, ...) {
	va_list args;
...
parse_keywords(const char *filename) {
	...
	for (i = j = 0, line = 1; i < (size_t) st.st_size; ++i) {
		...
		if (j >= (BUFSIZ - 1)) {
			bufname[j] = '\0';
			syntax(NULL, "word too long in %s:%d near '%s'", file->name, line,
					bufname);
			error = true;
			break;
		}
...
int get_conf(const char *filename) {
	struct conf_sec *s;
