...
static void status_gcache_free(struct status_ctx *ctx) {
...
static void status_graph_draw(struct status_ctx *ctx, struct status_seq *sq,
		struct status_gcache *gc) {
	int max = 0;
	int i, j, y;
	float c;
...
static void status_graph_process(struct status_ctx *ctx, struct status_seq *sq,
		char *name) {
	int j;
	struct status_gcache *gc;
...
static void status_apply_list(struct status_ctx *ctx) {
	...
	SLIST_FOREACH(sq, &ctx->statushead, next)
	{
		...
		case 's':
			...
			barwin_draw_text(ctx->barwin, ctx->theme, sq->geo.x, sq->geo.y,
					sq->fg, sq->str);
		...
		case 'P':
			...
			g.x = sq->geo.x
					+ ((sq->geo.w - sq->data[0])
							/ ((float) sq->data[2] / (float) sq->data[1]));
			g.y = sq->geo.y;
			g.w = sq->data[0];
			g.h = sq->geo.h;
...
void status_render(struct status_ctx *ctx) {
	...
	if (SLIST_EMPTY(&ctx->statushead)) {
		...
		barwin_draw_text(ctx->barwin, ctx->theme, ctx->barwin->geo.w - l,
				TEXTY(ctx->theme, ctx->barwin->geo.h), ctx->barwin->fg,
				ctx->status);
	} else
...
static void status_surface(int x, int y, int w, int h, BgColor bg, char *status) {
	struct barwin *b;
	struct screen *s;
	...
	if (x + y < 0)
		XQueryPointer(W->dpy, W->root, &rw, &rw, &x, &y, &d, &d,
				(unsigned int *) &d);
