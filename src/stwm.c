...
int stwm_error_handler(Display *d, XErrorEvent *event) {
	...
	if (event->error_code == BadAccess && W->root == event->resourceid)
	...
	if (client_gb_win(event->resourceid))
		if (event->error_code == BadWindow || event->request_code == 42
...
void stwm_init_font(char *font, struct theme *t) {
	...
	if (!(t->font.fontset = XCreateFontSet(W->dpy, font, &misschar, &d,
			&defstring))) {
		warnxl("Can't load font '%s'", font);
		t->font.fontset = XCreateFontSet(W->dpy, "fixed", &misschar, &d,
				&defstring);
	}
...
static void stwm_xinit(void) {
	XGCValues xgc = { .function = GXinvert, .subwindow_mode = IncludeInferiors,
			.line_width = 1 };

	XSetWindowAttributes at = { .event_mask = (KeyMask | ButtonMask | MouseMask
			| PropertyChangeMask | SubstructureRedirectMask
			| SubstructureNotifyMask | StructureNotifyMask), .cursor =
			XCreateFontCursor(W->dpy, XC_left_ptr) };
	...
	W->rgc = XCreateGC(W->dpy, W->root,
			GCFunction | GCSubwindowMode | GCLineWidth, &xgc);
...
void stwm_grab_keys(void) {
	...
	if ((c = XKeysymToKeycode(W->dpy, k->keysym))) {
		XGrabKey(W->dpy, c, k->mod, W->root, True, GrabModeAsync,
				GrabModeAsync);
		XGrabKey(W->dpy, c, k->mod | LockMask, W->root, True, GrabModeAsync,
				GrabModeAsync);
		XGrabKey(W->dpy, c, k->mod | W->numlockmask, W->root, True,
				GrabModeAsync, GrabModeAsync);
		XGrabKey(W->dpy, c, k->mod | LockMask | W->numlockmask, W->root, True,
				GrabModeAsync, GrabModeAsync);
	}
}
...
static void stwm_scan(void) {
	...
	if (XGetWindowProperty(W->dpy, W->root, W->net_atom[stwm_current_tag], 0,
			32, False, XA_CARDINAL, &rt, &rf, &ir, &il, (unsigned char**) &tret)
			== Success && tret) {
		nscreen = (int) ir;
	}
	...
	if (XGetWindowProperty(W->dpy, W->root, W->net_atom[stwm_focus], 0, 32,
			False, XA_WINDOW, &rt, &rf, &ir, &il, (unsigned char**) &ret)
			== Success && ret) {
		focus = *ret;
		XFree(ret);
	}
	...
	if (XQueryTree(W->dpy, W->root, &usl, &usl2, &w, (unsigned int*) &n))
		for (i = n - 1; i != -1; --i) {
			...
			if (!wa.override_redirect && wa.map_state == IsViewable) {
				...
				if (XGetWindowProperty(W->dpy, w[i], ATOM("_stwm_SCREEN"), 0,
						32, False, XA_CARDINAL, &rt, &rf, &ir, &il,
						(unsigned char**) &ret) == Success && ret) {
					screen = *ret;
					XFree(ret);
				}
				...
				if (XGetWindowProperty(W->dpy, w[i], ATOM("_stwm_TABMASTER"), 0,
						32, False, XA_WINDOW, &rt, &rf, &ir, &il,
						(unsigned char**) &ret) == Success && ret) {
					tm = *ret;
					XFree(ret);
				}
				...
				if (tag != -1 && screen != -1) {
					...
					if (getg
							&& tag
									<= TAILQ_LAST(&c->screen->tags, tsub)->id
											- 1)
...
static inline void stwm_sigchld(void) {
...
static inline void stwm_init(void) {
	log_init();
	stwm_xinit();
	ewmh_init();
	screen_init();
...
static void exec_uicb_function(Display *dpy, Window root, char *func, char *cmd) {
	...
	XClientMessageEvent e = { .type = ClientMessage, .message_type =
			XInternAtom(dpy, "_stwm_FUNCTION", false), .window = root, .format =
			32, .data.l[4] = true };
	...
	XChangeProperty(dpy, root, XInternAtom(dpy, "_stwm_CMD", false), utf8s, 8,
			PropModeReplace, (unsigned char*) cmd, strlen(cmd));
...
int main(int argc, char **argv) {
	...
	while ((i = getopt(argc, argv, "hvC:c:")) != -1) {
		...
		case 'c':
			...
			exec_uicb_function(dpy, DefaultRootWindow(dpy), optarg,
					argv[optind]);
