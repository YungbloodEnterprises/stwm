...
void systray_acquire(void) {
	Window w = 0;
	XSetWindowAttributes wattr = { .event_mask = ButtonPressMask | ExposureMask,
	...
	XChangeWindowAttributes(W->dpy, w,
			CWEventMask | CWOverrideRedirect | CWBackPixel, &wattr);
...
void systray_freeicons(void) {
	...
	XSetSelectionOwner(W->dpy, W->net_atom[net_system_tray_s], None,
			CurrentTime);
...
void systray_update(void) {
	...
	SLIST_FOREACH(i, &W->systray.head, next)
	{
		...
		XMoveResizeWindow(W->dpy, i->win, (i->geo.x = x), 0, i->geo.w,
				i->geo.h);
